...
class IPSymconExtension {
    ...
    async onMQTTMessage(data) {
        ...
        switch (data.topic) {
            case `${topicPrefix}/getDevices`:
                const devices = this.zigbee.devices(false).map(device => this.#createDevicePayload(device));
            ...
            case `${topicPrefix}/getDevice`:
                if (data.message) {
                    ...
                    await this.#publishToMqtt(`${device.name}/deviceInfo`, device);
                }
                break;
    ...
    #createDevicePayload(device) {
        ...
        return {
            ...
            exposes: device.definition?.exposes ?? 'No Exposes',
        };
    }
